...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      components={{
        Root: ({ className: rootClassName, rootRef, ...rootProps }: { className?: string; rootRef?: React.Ref<HTMLDivElement> }) => (
          <div data-slot="calendar" ref={rootRef} className={cn(rootClassName)} {...rootProps} />
        ),
        Chevron: ({ className: chevronClassName, orientation, ...chevronProps }: { className?: string; orientation?: string }) => {
          if (orientation === "left") {
...
type CalendarDayButtonProps = React.ComponentProps<typeof Button> & {
  day: { date: Date }
  modifiers: { focused?: boolean; selected?: boolean; range_start?: boolean; range_end?: boolean; range_middle?: boolean }
}
